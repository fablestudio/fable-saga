...
class GoTo(SimAction):
    ...
    async def tick(self, delta: timedelta, sim: Simulation):
        ...
        self.agent.memories.append(
            sim_models.Memory(summary=summary, timestamp=sim.sim_time)
        )
...
class Interact(SimAction):
    ...
    async def tick(self, delta: timedelta, sim: Simulation):
        ...
        self.agent.memories.append(
            sim_models.Memory(
                summary=f"Interacted with {self.item_guid} via {self.interaction}"
...
class Wait(SimAction):
    ...
    async def tick(self, delta: timedelta, sim: Simulation):
        ...
        self.agent.memories.append(
            sim_models.Memory(
                summary=f"Waited for {self.duration}m "
...
class Reflect(SimAction):
    ...
    async def tick(self, delta: timedelta, sim: Simulation):
        ...
        formatted_reflection = (
            f"Reflected while " + f"at {self.agent.location.guid}: " + f"{reflection}"
        )

        self.agent.memories.append(
            sim_models.Memory(summary=formatted_reflection, timestamp=sim.sim_time)
        )
        self.end_time = sim.sim_time
...
class ConverseWith(SimAction):
    ...
    def __init__(self, agent: SimAgent, action_data: fable_saga.Action):
        ...
        self.persona_guid = self.parameters[
            "persona_guid"
        ]  # The other person to speak with
        self.topic = self.parameters["topic"]
        self.context = self.parameters["context"]
        self.goal = self.parameters.get(
            "goal", ""
        )  # sometimes goal doesn't get generated, so default to empty string.

    async def tick(self, delta: timedelta, sim: Simulation):
        await super().tick(delta, sim)

        # Generate and format the conversation into a memory
        generated_conversation = await sim.conversation_generator.generate_conversation(
            sim, self.agent, self.persona_guid, f"[TOPIC] \n {self.topic}"
        )
        if generated_conversation.error is None:
            ...
            sim.agents[self.persona_guid].memories.append(
                sim_models.Memory(
            ...
            self.end_time = sim.sim_time
            print(
                f"{self.agent.persona.id()} conversed with {self.persona_guid} on {self.topic}.\n"
                f"{conversation_formatted}"
            )
        else:
