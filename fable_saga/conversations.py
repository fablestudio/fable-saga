...
from . import default_openai_model_name, default_openai_model_temperature, logger, SagaCallbackHandler
...
class ConversationAgent:
    ...
    async def generate_conversation(self, persona_guids: List[str], context: str, max_tries=0,
                                    verbose=False, model_override: Optional[str] = None):
        ...
        while retries <= max_tries:
            try:
                last_response = await chain.arun(context=context, persona_guids=formatted_persona_guids,
        ...
        return GeneratedConversation(conversation=[], retries=retries, raw_response=callback_handler.last_generation,
